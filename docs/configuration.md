...
| `VLLM_METAL_EXPERIMENTAL_GDN_APC` | `0` | Enable correctness-first scheduler-coherent APC checkpoints for text-only hybrid GDN models. Requires paged attention, prefix caching, `mamba_cache_mode=align`, chunked local prefill, TP=PP=DP=1, and no speculative decoding, LoRA, TurboQuant, KV transfer, external KV connectors, or disaggregated prefill. |
